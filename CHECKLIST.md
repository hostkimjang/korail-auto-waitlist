...
- [x] 모든 화면 푸터의 릴리스 버전·업데이트 날짜(요일) 표시와 저장소 `CHANGELOG.md` 맨 위 항목 일치 회귀
...
- [x] replay `source_unavailable`에 URL·쿠키·본문 없는 운영 진단 분류(`detail`)를 추가하고 `cold_reinit` 로그에 노출
- [x] 브라우저 전용 관측의 실측 비용을 반영해 `관측 지연` 안내 유예를 30초에서 300초로 조정
- [x] 공식 페이지에 도달하지 못한 `official_read_unavailable` 확인이 제한된 재확인 횟수를 소비하지 않도록 수정하고 회귀 테스트 추가
- [x] 운영사 응답에 감시 좌석 등급이 없는 경우를 `provider_result_incomplete`로 분리해 공식 출처 장애와 구분하고 화면 문구를 `목록 불완전`으로 표시
- [ ] 활성 그룹 수와 최근 조회 비용에서 관측 목표(`next_check_at`)를 계산해 달성 가능한 값으로 만들기. 현재는 관리자 설정 주기를 그대로 써서 목표 자체가 달성 불가능하고, 지연 유예 상수로만 보정하고 있음
...
- [ ] 재배포 뒤 실제 사용 기기의 브라우저와 설치형 PWA에서 푸터 릴리스 버전이 새 값으로 바뀌는지 확인
...
- [x] 2026년 9월 17일 접속 대기 확장 수정을 Oracle에 `experimental-rail` 프로필 전체로 재빌드·재생성하고 migration 정상 종료와 장기 서비스 12개 `healthy`를 확인. 배포 직후 10분 동안 `event=result_expansion_stopped`·`event=official_connection_wait_timeout` 없이 조회가 성공
- [ ] 실제 혼잡 시간대의 `서비스 연결대기` 상황에서 결과 확장이 끝까지 진행되고 `event=result_expansion_stopped`가 남지 않는지 운영 표본으로 확인. 배포 시점에는 접속 대기가 발생하지 않아 실제 대기 상황을 통과시키지 못했음
