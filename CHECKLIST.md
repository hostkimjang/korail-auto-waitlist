...
- [x] 요청 hash·idempotency 조회/추가를 FastAPI 비의존 `idempotency/application.py`로 이동하고 `IdempotencyConflict`를 watch create/start/pause/cancel·official evidence HTTP에서만 기존 detail 409로 변환. services 세 함수 identity와 internal canonical binding, official confirmation 직접 import를 유지해 services를 1,417→1,383줄로 축소
...
- [x] 198줄 `watch_management/transition_application.py`에 아홉 typed port와 FastAPI 비의존 rejection을 두고 replay→policy→status/clock/cooldown/provider→history→idempotency→status outbox→알림 orchestration을 이동. `services.apply_watch_transition`은 호출 시 globals를 조립해 기존 signature/identity/monkeypatch seam과 exact HTTP 409만 보존하고 1,399→1,368줄로 축소
- [x] 순수 policy와 application owner 테스트를 343+10회 상당으로 보존·보강하고 missing replay·transport-independent rejection·clock/provider 1회·observation history·artifact 순서·caller commit/rollback을 고정. 인접 pytest 499건·API 전체 1,484건·Ruff `E/F/I`·format ratchet 60개·strict mypy 14개·lock check·독립 리뷰 P0~P3 없음·`git diff --check` 통과
- [x] 스물다섯 번째 구조 슬라이스 B의 `experimental-rail` 전체 build·force-recreate 후 migration·log-init exit 0, 장기 서비스 11개 healthy, API health·ready와 proxy health 200, 최근 안전한 오류 표식 0건 확인
...
- [x] 273줄 `watch_management/update_application.py`로 watch 수정 명령의 잠금·상태별 수정 필드·채널·focused 정원·후보/시간창 검증과 mutation→due 재무장→dedupe→outbox→commit→refresh를 이동. `services.update_watch`와 validator는 기존 identity·monkeypatch seam·404/409/422 facade만 유지하고 `services.py`를 1,368→1,253줄로 축소
- [x] watch update owner 15건·owner/boundary 28건, API 전체 pytest 1,500건, Ruff `E/F/I`, format ratchet 60개, strict mypy 15개 오류 0, lock check와 독립 리뷰 P0~P3 없음 확인
- [x] 스물다섯 번째 구조 슬라이스 E의 `experimental-rail` 전체 build·force-recreate 후 migration·log-init exit 0, 장기 서비스 11개 healthy, API health·ready와 proxy health 200, 최근 안전한 오류 표식 0건 확인
...
- [x] 53줄 `reservations/attempt_policy.py`에 field Protocol 기반 confirmed-absent 재무장과 결제 보류 종료 retry-edge 상태를 단일화하고 observation group의 중복 episode prefix 제거. 183줄 `attempt_claim_application.py`에 replay·latest fence·재시도 자격·savepoint/IntegrityError 승자 재조회·상태/outbox를 caller UoW로 이동하며 `services.py`를 1,253→1,131줄로 축소
- [x] services claim wrapper의 기존 signature/module identity·호출 시점 monkeypatch seam과 begin_nested/flush·commit/rollback/refresh 부재를 고정. owner/boundary 29건·관련 75+2건·API 전체 1,516건·Ruff `E/F/I`·format ratchet 60개·strict mypy 17개·lock check·독립 리뷰 P0~P3 없음 확인
