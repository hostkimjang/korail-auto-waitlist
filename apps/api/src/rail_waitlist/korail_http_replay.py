...
_PROTECTION_MARKERS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("code_1405", re.compile(r"(?<!\d)-?1405(?!\d)", re.IGNORECASE)),
    ("code_8002", re.compile(r"(?<!\d)-?8002(?!\d)", re.IGNORECASE)),
    ("code_8003", re.compile(r"(?<!\d)-?8003(?!\d)", re.IGNORECASE)),
    ("macro_err", re.compile(r"macro_err", re.IGNORECASE)),
    ("captcha", re.compile(r"captcha", re.IGNORECASE)),
    ("netfunnel", re.compile(r"netfunnel", re.IGNORECASE)),
    ("unauthorized", re.compile(r"미허가", re.IGNORECASE)),
    ("restricted", re.compile(r"이용\s*제한", re.IGNORECASE)),
)
_SESSION_MARKERS = (
...
class KorailHttpReplayProtectionDetected(KorailHttpReplayError):
    def __init__(self, trigger: str = "http_403") -> None:
    ...
    for row in rows:
        ...
        trains.append(
            BrowserTrainSnapshot(
                ...
                expected_delay_minutes=_expected_delay_minutes(
                    row.get("h_expn_dpt_dlay_tnum")
                ),
            )
        )
...
def _protection_marker(body: bytes) -> str | None:
    text = body.decode("utf-8", errors="ignore")
    for marker, pattern in _PROTECTION_MARKERS:
