...
from .srt_fullstack_fixture import fullstack_srt_client_factory
from .srt_seat_source import SrtClientFactory, SrtLiveSeatSource
...
def _source_for_config(config: SrtExecutionSourceConfig) -> ManagedSrtSeatObserver:
    ...
    if config.fixture_url is None:
        ...
        source = SrtLiveSeatSource(
            ...
            client_factory=cast(
                SrtClientFactory,
                fullstack_srt_client_factory(config.fixture_url),
            ),
