...
from SRT import SRTError, SRTLoginError, SRTResponseError
from SRT.errors import SRTNetFunnelError

from .domain import Provider
from .korail_reservation_contract import KorailSessionStateResult
...
class ProviderLoginVerifier:
    ...
    def __init__(
        ...
        self._korail = korail
        self._srt = srt or default_srt_reservation_executor()
    ...
    async def _verify_srt(
        ...
        *,
        ...
        try:
            method = (
                self._srt.prewarm_credentials if prewarm else self._srt.verify_credentials
            )
        ...
        if provider is Provider.SRT:
            ...
            if callable(remote_status):
                ...
                return ProviderSessionRuntimeSnapshot(
                    ...
                    created_age_seconds=getattr(raw, "created_age_seconds", None),
                    last_verified_age_seconds=getattr(
                        raw,
                        "last_verified_age_seconds",
                        None,
                    ),
                    last_used_age_seconds=getattr(raw, "last_used_age_seconds", None),
                    local_reuse_remaining_seconds=getattr(
                        raw,
            ...
            raw = self._srt.session_snapshot()
            now = time.monotonic()
            ...
            return ProviderSessionRuntimeSnapshot(
                ...
                created_age_seconds=age(raw.created_at_monotonic),
                last_verified_age_seconds=age(raw.last_verified_at_monotonic),
                last_used_age_seconds=age(raw.last_used_at_monotonic),
                local_reuse_remaining_seconds=(
                    None
                    if raw.local_reuse_until_monotonic is None
                    else max(0.0, raw.local_reuse_until_monotonic - now)
                ),
                locally_reusable=raw.locally_reusable,
            )
        return ProviderSessionRuntimeSnapshot(
