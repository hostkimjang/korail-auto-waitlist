...
def _updated_exactly_one_row(result: object) -> bool:
    rowcount: object = getattr(result, "rowcount", None)
...
class StationCatalogRepository:
    ...
    async def load_snapshot(self) -> StationCatalogSnapshot | None:
        ...
        async with self._session_factory() as session:
            ...
            if not isinstance(payload, dict):
                return None
            raw_identity_stations: object = payload.get("stations")
            raw_display_stations: object = payload.get("display_stations")
            raw_visibility: object = payload.get("visibility")
            if not isinstance(raw_visibility, dict):
                return None
            raw_visibility_retrieved_at: object = raw_visibility.get("retrieved_at")
            if not isinstance(raw_visibility_retrieved_at, str):
                return None
            try:
                ...
                visibility_retrieved_at = datetime.fromisoformat(raw_visibility_retrieved_at)
            except (TypeError, ValidationError, ValueError):
                return None
            if (
                raw_visibility.get("source") != "korail_station_guide"
                or raw_visibility.get("url") != self._visibility_url
    ...
    async def try_acquire_lease(self, owner: str, now: datetime, lease_until: datetime) -> bool:
        ...
        async with self._session_factory() as session:
            ...
            await session.commit()
            return _updated_exactly_one_row(result)
