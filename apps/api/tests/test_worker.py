...
async def test_late_night_seat_found_watch_uses_selected_departure_not_next_day_arrival(
    app, db_engine, monkeypatch
):
    monkeypatch.setattr(worker_module, "SessionFactory", app.state.test_session_factory)
    adapter = TaskScopedSoldOutAdapter(Provider.SRT)
    korea = timezone(timedelta(hours=9))
    service_day = (datetime.now(korea) + timedelta(days=2)).date()
    local_now = datetime.combine(service_day, time(23, 50), tzinfo=korea)
    ...
    async with factory() as session:
        watch = Watch(
            ...
            destination="부산",
            destination_node_id="N-BUSAN",
            travel_date=service_day,
            time_from=time(23, 45),
            time_to=time(0, 55),
            passenger_count=1,
            status=WatchStatus.SEAT_FOUND,
            mode="official",
            dedupe_key="late-night-seat-found",
    ...
    await _process_watch_group([watch_id], now, provider=Provider.SRT, adapter=adapter)
    async with factory() as session:
        watch = await session.get(Watch, watch_id)
        assert watch.status is WatchStatus.WATCHING
        assert watch.next_check_at is not None
...
async def test_seat_found_watch_expires_after_unknown_operational_horizon(
    ...
    async with factory() as session:
        watch = Watch(
            ...
            travel_date=local_departure.date(),
            time_from=(local_departure - timedelta(hours=1)).time(),
            time_to=(local_departure + timedelta(hours=2)).time(),
            passenger_count=1,
            status=WatchStatus.SEAT_FOUND,
            mode="official",
            dedupe_key="expire-at-service-departure",
            next_check_at=now - timedelta(seconds=1),
        )
        watch.candidates.append(
            WatchCandidate(
    ...
    async with app.state.test_session_factory() as session:
        assert await worker_module._expire_elapsed_watches(session, now) == 1
...
async def test_elapsed_scheduled_departure_with_fresh_delay_is_not_expired(
    app, db_engine, monkeypatch
):
    monkeypatch.setattr(worker_module, "SessionFactory", app.state.test_session_factory)
    now = datetime.now(timezone.utc).replace(microsecond=0)
    scheduled_departure = now - timedelta(hours=7)
    local_departure = scheduled_departure.astimezone(timezone(timedelta(hours=9)))
    factory = async_sessionmaker(db_engine, expire_on_commit=False)
    async with factory() as session:
        watch = Watch(
            provider=Provider.MOCK,
    ...
    await _process_watch_group([watch_id], now, provider=Provider.MOCK, adapter=adapter)
    assert adapter.observe_calls == 1

    async with factory() as session:
        watch = await session.get(Watch, watch_id)
        candidate = watch.candidates[0]
        assert watch.status is not WatchStatus.EXPIRED
        assert candidate.state != "expired"


async def test_fresh_closed_booking_window_expires_before_scheduled_departure(
    app, db_engine, monkeypatch
):
    monkeypatch.setattr(worker_module, "SessionFactory", app.state.test_session_factory)
    # 22:30 KST + two hours crosses the service-date boundary. A fresh official
    # CLOSED signal still has to expire this watch even though travel_date is tomorrow.
    ...
    async with factory() as session:
        watch = Watch(
            ...
            status=WatchStatus.WATCHING,
            mode="official",
            dedupe_key="fresh-closed-booking-window",
        )
        watch.candidates.append(
            WatchCandidate(
                train_number="MOCK-CLOSED",
                departure_at=scheduled_departure,
                scheduled_departure_at=scheduled_departure,
                seat_class=SeatClass.STANDARD,
                priority=1,
