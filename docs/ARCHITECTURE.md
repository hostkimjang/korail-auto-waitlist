...
features -> operations -> app-surfaces -> features/official-handoff/officialHandoff ->
features/new-wait/officialSeatConfirmation -> responsive` 순서의 아홉 경계를 import합니다. 초기 다섯 경계
...
trigger·dialog·760px/340px 규칙은 `features/new-wait/officialSeatConfirmation.css`에 둡니다.
`.official-handoff-note`, `.copy-status`, `.provider-chip`, button primitive는 기존 공용 owner에 남깁니다.
...
전역 token을 사용합니다. 기본 Playwright responsive 검사는 1,440px·320px·720px viewport에서 실제
`OfficialHandoff` dialog를 열어 panel 경계·가로 넘침 없음·세 행동 영역 44px 이상·app shell의 inert
전환·Escape 닫기 뒤 trigger focus 복원을 확인합니다. 이는 production `OfficialHandoff`의 직접 검증이며
dormant `OfficialSeatConfirmation`의 직접 dialog 검증과는 구분합니다. 나머지 feature별 CSS 재소유,
selector 중복 정리나 CSS Modules 전환은 이 동작 보존 체크포인트와 섞지 않는 별도 슬라이스로 다룹니다.
...
selector를 소유합니다. strict `features/settings/useUiPreferencesSettings.ts`는 인증 뒤 화면 환경설정
조회, demo/live 저장, saving 상태와 logout reset을 소유합니다. App은 polling 활성 여부와 watch
collection·새 대기·설정 화면 props를 연결합니다.
...
`apply_watch_transition` 공개 wrapper는 호출 시점의 dependency를 조립하고 금지 전이만 기존 HTTP 409로
변환합니다. canonical orchestration은 FastAPI 비의존 `watch_management/transition_application.py`가
소유합니다. 이 application은 idempotency replay를 먼저 확인한 뒤 정책을 해석해 status·UTC timestamp를
적용하고, SCHEDULED 허용 전이에만 provider capability를 한 번 조회합니다. history → idempotency →
`watch.status_changed` outbox → 채널 알림 순서와 같은 UoW를 유지하며 lock·commit·refresh·rollback은
`transition_watch` 또는 관찰·예약 caller에 남습니다.
...
역명·KST 시간창·공식 미관측 좌석 정책은 `provider_adapters/timetable_support.py`, TAGO response
envelope·pagination parser와 `TagoClient`·process singleton은 `provider_adapters/tago.py`, 공식
...
공통 credential loader는 `provider_adapters/execution.py`, KORAIL background 실행은
`provider_adapters/korail_execution.py`, SRT background 실행은
