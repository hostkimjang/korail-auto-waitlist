...
2026-08-04 구조 진단 착수 기준 주요 구조 부채는 `App.jsx` 약 2,100줄, `api.js` 1,185줄, `styles.css` 약 6,670줄입니다. `NewWait`는 여전히 폼·역 카탈로그·시간표 요청·stale response 차단·열차 선택·단계 렌더링의 최종 조립을 담당하고, `App`은 인증·SSE·작업 CRUD·알림·화면 전환을 함께 담당합니다. 줄 수는 분리 목표가 아니라 서로 다른 변경 이유가 집중된 위치를 찾는 지표로만 사용합니다.
...
5. 기능 상태 분리
   ...
   - `NewWait`의 form reducer, station catalog, timetable search, selection priority hook
...
`dist/client/index.html`, `dist/server/index.js`, `dist/.openai/hosting.json`을 유지합니다. `any`, `@ts-ignore`, 타입검사 제외 범위 증가, 테스트 수 감소로 전환을 통과시키지 않습니다.
