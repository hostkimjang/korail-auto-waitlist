...
`demo-capture-motion.mjs`는 접근성 선택자로 찾은 실제 위치를 기준으로 커서 이동과 클릭 링을 그립니다. 핵심 행동에서만 화면 스냅샷을 확대했다가 결과 화면으로 복귀하므로 앱 화면 구조와 스크롤 동작에는 영향을 주지 않습니다.
...
ffmpeg -y -ss 00:00:13.50 -i docs/media/railwait-demo.mp4 -frames:v 1 -vf "scale=1280:-2:flags=lanczos" docs/media/railwait-demo-poster.png
...
공개 영상은 3초짜리 브랜드 장면에서 실제 UI로 이어지는 21.76초 구성입니다. 원본 녹화 앞의 브라우저 준비 화면 2.36초는 합성할 때 제외합니다.
